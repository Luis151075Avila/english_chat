...
// FIX: Using named import for EventEmitter to fix type resolution issues.
...
// FIX: GenAILiveClient was not extending EventEmitter, causing errors on `on`, `off`, and `emit`.
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      // FIX: Add event emitter call for error
  ...
  protected onMessage(message: LiveServerMessage) {
    if (message.setupComplete) {
      // FIX: Add event emitter call for setupcomplete
      this.emit('setupcomplete');
      return;
    }
    if (message.toolCall) {
      this.log('server.toolCall', message);
      // FIX: Add event emitter call for toolcall
      this.emit('toolcall', message.toolCall);
      return;
    }
    if (message.toolCallCancellation) {
      this.log('receive.toolCallCancellation', message);
      // FIX: Add event emitter call for toolcallcancellation
    ...
    if (message.serverContent) {
      ...
      if ('interrupted' in serverContent) {
        ...
        // FIX: Add event emitter call for interrupted
        this.emit('interrupted');
        return;
      }
      if ('turnComplete' in serverContent) {
        this.log('server.send', 'turnComplete');
        // FIX: Add event emitter call for turncomplete
      ...
      if (serverContent.modelTurn) {
        ...
        base64s.forEach(b64 => {
          if (b64) {
            ...
            // FIX: Add event emitter call for audio
        ...
        // FIX: Add event emitter call for content
  ...
  protected onOpen() {
    this._status = 'connected';
    // FIX: Add event emitter call for open
    this.emit('open');
  }
  ...
  protected onClose(e: CloseEvent) {
    ...
    // FIX: Add event emitter call for close
    this.emit('close', e);
  }
  ...
  protected log(type: string, message: string | object) {
    // FIX: Add event emitter call for log
    this.emit('log', {
      type,
      message,
      date: new Date(),
