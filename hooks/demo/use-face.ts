...
export function useBlink({ speed, phase = 0 }: BlinkProps) {
  ...
  useEffect(() => {
    function nextFrame() {
      frameId.current = window.requestAnimationFrame(() => {
        setFrame(frame + 1);
        let s = easeOutQuint((Math.sin(frame * speed + phase) + 1) * 2);
  ...
  }, [speed, eyeScale, frame, phase]);

  return eyeScale;
}

export default function useFace(): FaceResults {
  const { volume } = useLiveAPIContext();
  const leftEyeScale = useBlink({ speed: 0.0125, phase: 0 });
  const rightEyeScale = useBlink({ speed: 0.0128, phase: Math.PI / 2 });

  return { leftEyeScale, rightEyeScale, mouthScale: volume / 2 };
}
